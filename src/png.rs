...
use crate::chunk::{buffer_u32, Chunk};
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> core::result::Result<Self, Self::Error> {
        let mut length_of_input = value.len() - 8;
        let mut vec_of_chunks: Vec<Chunk> = Vec::new();
        ...
        while length_of_input > 0 {
            ...
            let mut total_bytes_to_read =
                usize::try_from(buffer_u32(&value[start..start + 4]).unwrap())
                    .map_err(|_| "Error in converting to usize")?;

            total_bytes_to_read = total_bytes_to_read + 12;

            if (total_bytes_to_read > length_of_input) {
                break;
            }
            length_of_input = length_of_input - total_bytes_to_read;

            vec_of_chunks.push(
                Chunk::try_from(&value[start..(start + total_bytes_to_read)])
                    .map_err(|_| "Can't push into Vec<Chunk>, type maybe bad")?,
            );

            start = start + total_bytes_to_read;
        }
        //header
        let mut png = Png {
            sign_header: Default::default(),
            // sign_header: value[0..8],
            chunks: vec_of_chunks,
        };

        png.sign_header.copy_from_slice(&value[0..8]);
        if *png.header() == Png::STANDARD_HEADER && length_of_input == 0 {
            Ok(png)
        } else {
            Err("Can't convert &[u8] to Png")
        }
...
impl Png {
    ...
    fn from_chunks(chunks: Vec<Chunk>) -> Png {
        Png {
            sign_header: Png::STANDARD_HEADER,
            chunks,
        }
    }
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let chunk_type_arg = ChunkType::from_str(chunk_type)?;

        let index = self
            .chunks
            .iter()
            .position(|x| (*x).chunk_type == chunk_type_arg)
            .unwrap();
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        let chunk_type_arg = ChunkType::from_str(chunk_type).unwrap();
        let index = self
            .chunks
            .iter()
            .position(|x| (*x).chunk_type == chunk_type_arg);
        ...
        match index {
            Some(x) => Some(&self.chunks[x]),
            None => None,
        }
    }
    pub fn as_bytes(&self) -> Vec<u8> {
        let chunk_vec: Vec<u8> = self
            .chunks
            .iter()
        ...
        Png::STANDARD_HEADER
            .iter()
            .chain(chunk_vec.iter())
            .copied()
            .collect()

        // dbg!(self.chunks.iter().map(|x| x.as_bytes()).collect());
        // Chunk::as_bytes(&self.chunks).copied().collect();
    }
}
